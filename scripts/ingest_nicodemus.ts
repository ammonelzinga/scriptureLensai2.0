...
type AIPearlChunk = { chapter_numbers: number[]; verse_numbers: number[]; combined_text: string; verses: { chapter: number; verse: number }[] };
type Chunk = {
  id: string;
  chunk_index: number;
  text: string;
...
function readNicodemusNormalized(filePath: string): Verse[] {
  const raw = JSON.parse(fs.readFileSync(filePath, 'utf-8')) as BookFile;
  const book = raw.book ?? 'Gospel of Nicodemus';
  const verses: Verse[] = [];
...
async function aiChunkChapter(openaiClient: OpenAI, bookName: string, chapterNumber: number, verses: Verse[], cacheDir?: string, strictJson = false, maxRetries = 2): Promise<AIPearlChunk[]> {
  ...
  if (cachePath && fs.existsSync(cachePath)) {
    try { return JSON.parse(fs.readFileSync(cachePath, 'utf-8')) as AIPearlChunk[]; } catch {}
  }
  const userPayload = JSON.stringify({ book: bookName, chapter: chapterNumber, verses: verses.map((v) => ({ verse: v.verse, text: v.text })) });
  const model = process.env.CHUNKING_MODEL ?? process.env.OPENAI_CHAT_MODEL ?? 'gpt-4.1';
  let arr: any[] = [];
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    const resp = await openaiClient.chat.completions.create({ model, temperature: 0, messages: [ { role: 'system', content: CHUNKING_PROMPT }, { role: 'user', content: userPayload } ], ...(strictJson ? { response_format: { type: 'json_object' } } : {}) });
    const content = resp.choices[0]?.message?.content ?? '';
    if (rawPath) {
      const dir = path.dirname(rawPath); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); fs.writeFileSync(rawPath, content);
    }
    let parsed: any; try { parsed = JSON.parse(content); } catch { parsed = { chunks: [] }; }
    arr = Array.isArray(parsed) ? parsed : (parsed.chunks ?? []);
    if (Array.isArray(arr) && arr.length > 0) break;
    await new Promise((r) => setTimeout(r, 300 + Math.floor(Math.random() * 250)));
  }
  if (cachePath) { const dir = path.dirname(cachePath); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); fs.writeFileSync(cachePath, JSON.stringify(arr, null, 2)); }
  return arr as AIPearlChunk[];
}

async function ensureLineageAndBook(supabase: any) {
  const tName = 'Christian';
  const sName = 'Apocrypha';
  const wName = 'Gospel of Nicodemus';
  const bTitle = 'Gospel of Nicodemus';
  ...
  const bId = (bRows && bRows[0]?.id) || (await supabase.from('books').insert([{ title: bTitle, work_id: wId, seq: 1 }] as any).select('id')).data?.[0]?.id;
  return { traditionId: tId, sourceId: sId, workId: wId, bookId: bId, bookTitle: bTitle };
}
...
async function upsertVersesForChunks(supabase: any, embedded: Chunk[], bookId: string, bookName: string, chunkIdByHash: Map<string, string>, verses: Verse[]) {
  ...
  let missingChunkIdRefs = 0;
    const verseRows: Array<{ book_id: string; chapter_seq: number; verse_seq: number; text: string; chunk_id: string }> = [];
  for (const c of embedded) {
    const refs = c.verse_ids.map((id) => { const parts = id.split(':'); return { chapter: parseInt(parts[1], 10), verse: parseInt(parts[2], 10) }; });
    ...
    const chunkUuid = chunkIdByHash.get(combined_hash); if (!chunkUuid) continue;
    for (const r of refs) {
      const text = textByKey.get(`${bookName}:${r.chapter}:${r.verse}`) ?? '';
        const key = `${bookId}|${r.chapter}|${r.verse}`;
        // Prefer first association; avoid multiple rows for same constrained key in one batch
        if (!uniq.has(key)) {
          uniq.set(key, { book_id: bookId, chapter_seq: r.chapter, verse_seq: r.verse, text, chunk_id: chunkUuid });
        }
    }
  }
  if (missingChunkIdRefs) console.warn(`Skipped ${missingChunkIdRefs} verse refs due to missing chunk UUID mapping.`);
    const rows = Array.from(uniq.values());
    if (rows.length) {
    console.log(`Upserting ${rows.length} verse rows to Supabase...`);
    const { error } = await supabase.from('verses').upsert(rows as any, { onConflict: 'book_id,chapter_seq,verse_seq' });
    if (error) throw error;
    console.log(`Upserted ${rows.length} verses.`);
  }
}

async function main() {
  const args = process.argv.slice(2);
  const normalizedPath = args.includes('--inputFile') ? args[args.indexOf('--inputFile') + 1] : 'data/nicodemus_normalized.json';
  const outDir = args.includes('--outDir') ? args[args.indexOf('--outDir') + 1] : 'data/chunks_nicodemus';
  const cacheRoot = 'data/chunks_cache_nicodemus';
  const strictJson = args.includes('--strictJson');
  const doEmbed = args.includes('--embed');

  const verses = readNicodemusNormalized(normalizedPath);
  const bookName = 'Gospel of Nicodemus';
  const chapters = new Map<number, Verse[]>();
  for (const v of verses) { if (!chapters.has(v.chapter)) chapters.set(v.chapter, []); chapters.get(v.chapter)!.push(v); }
  ...
  for (const [chNum, chVerses] of [...chapters.entries()].sort((a,b)=>a[0]-b[0])) {
    ...
    for (const c of aiChunks) {
      ...
      chunks.push({ id, chunk_index: chunkIndex, text, token_count, book_id: firstVerse.book_id, book_name: firstVerse.book_name, start_chapter: firstRef.chapter, start_verse: firstRef.verse, end_chapter: lastRef.chapter, end_verse: lastRef.verse, verse_ids: refs.map((r) => `${firstVerse.book_id}:${r.chapter}:${r.verse}`) });
      chunkIndex++;
    }
  }

  if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
  const payload = { book: bookName, chunks: chunks.map((c) => ({ chapter_numbers: [c.start_chapter, c.end_chapter].filter((v, i, a) => a.indexOf(v) === i), verse_numbers: c.verse_ids.map((id) => parseInt(id.split(':')[2], 10)), combined_text: c.text, verses: c.verse_ids.map((id) => { const parts = id.split(':'); return { chapter: parseInt(parts[1], 10), verse: parseInt(parts[2], 10) }; }) })) };
  const outPath = path.join(outDir, `${bookName}.json`);
  fs.writeFileSync(outPath, JSON.stringify(payload, null, 2));
  console.log(`Wrote Nicodemus chunks to ${outPath} (${chunks.length} chunks).`);
  ...
  if (doEmbed) {
    ...
    const { bookId } = await ensureLineageAndBook(supabase);

    const rowsRaw = embedded.map((c) => {
      const refs = c.verse_ids.map((id) => { const parts = id.split(':'); return { chapter: parseInt(parts[1], 10), verse: parseInt(parts[2], 10) }; });
      const chapter_numbers = refs.map((r) => r.chapter);
      const verse_numbers = refs.map((r) => r.verse);
      const combined_text = c.text;
      const combined_hash = crypto.createHash('sha1').update(`${bookId}|${chapter_numbers.join(',')}|${verse_numbers.join(',')}`).digest('hex');
      return { book_id: bookId, start_chapter: c.start_chapter, end_chapter: c.end_chapter, verse_numbers, chapter_numbers, combined_text, embedding: c.embedding, combined_hash };
    });
    // De-duplicate by combined_hash to avoid ON CONFLICT affecting same row twice
    const uniqMap = new Map<string, any>();
    for (const r of rowsRaw) {
      if (!uniqMap.has(r.combined_hash)) uniqMap.set(r.combined_hash, r);
    }
    ...
    if (error) throw error;
    console.log(`Upserted ${data?.length ?? 0} Nicodemus chunks to Supabase.`);

    const map = new Map<string, string>();
    for (const r of data ?? []) map.set(r.combined_hash, r.id);
    await upsertVersesForChunks(supabase, embedded, bookId, bookName, map, verses);
    console.log(`Upserted verses and ensured chapters for Nicodemus.`);
  }
}
// Better error handling for promise rejections
process.on('unhandledRejection', (reason) => {
  try {
    console.error('UnhandledPromiseRejection:', JSON.stringify(reason, null, 2));
  } catch {
